...
exports.run = async function (client, message, args) {
  if (
    ...
    message.author.id === "302457454846017546"
  ) {
    if (args[0] === "all") {
      ...
      for (i = args[1]; i < args[2]; i++) {
        ...
        if (fserver === null) {
          ...
          await message.reply(
            "`" + CguildName[i] + "` is already in the databse"
          );
        }
      }
      return;
    ...
    return message.reply(
      "Baby users with admin permissions can add this server to the database!"
    );
  }
};
